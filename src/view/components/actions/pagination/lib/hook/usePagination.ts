...
export const usePagination = (
  ...
  const [selectValue, arraySelect, onChangeArraySelect] = useSelectPagination(selectOptionValue, arrayOptionSelect, onCallbackSelect)
  ...
  if(maxNumberCount > maxPageNumber) {
    ...
    for(let i = 0, j = maxNumber - (maxPageNumber - 1); j <= maxNumber; i++, j++) {
      arrayButton[i] = j
    }
  } else {
  ...
  const isPaginationButton = (sign: signType) => {
    const calc = {
      '+': (!(maxNumberCount <= arrayButton[arrayButton.length - 1] + maxPageNumber)),
